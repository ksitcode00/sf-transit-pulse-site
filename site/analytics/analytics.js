...
const COPY = {
  en: {
    ...
    scope:
      "Features 1–3 use the latest complete historical Muni month. Feature 4 is collecting predictions. Feature 5 maps current street work. Feature 6 is archiving notices. Features 7–8 show crash history and incident reports, not personal safety predictions.",
    ...
    exploreIncidents: "Explore incident history →",
  ...
  zh: {
    ...
    scope:
      "功能 1–3 使用最新完整的 Muni 历史月份；功能 4 正在积累预测；功能 5 显示当前道路施工；功能 6 正在归档公告。功能 7–8 提供事故历史与事件报告，不预测个人安全。",
...
async function load() {
  ...
  if (firstLoad) {
    ...
    if (["overview", "historical-service", "eta-accuracy", "construction-exposure", "traffic-safety", "incident-environment", "methodology"].includes(id))
