...
| 系统层 | 具体做什么 | 为什么重要 |
|---|---|---|
| 实时数据采集 | 定时获取车辆位置、班次预测、服务通知、道路事件、停车付费活动和历史事件记录。 | 作者不需要一直开着 Notebook 或电脑，任何人都能直接使用公开网站。 |
...
| 多目标排序 | “最快到达”“综合推荐”“安全优先”从同一组可行路线中，按公开规则分别排序。 | 用户偏好只改变排名，不会偷偷修改路线本身的预计时间。 |
...
| 当前回报速度 | 帮助判断车辆当前移动是否明显低于同类交通方式的透明参考值；参考值不会冒充历史平均速度。 | 某段公交走廊当前中位速度明显偏低时，用户会看到减速提示。 | 有速度数据时上线；走廊历史基线安排在 v1.1 |
...
| 安全优先 | 把去重后的 30、90、365 天报告与全部 Muni 站点比较。直达权重为起点/上车 45%、沿途 40%、目的地 15%；换乘路线为 20%、35%、30%、15%。 | 夜间出行时，把沿途历史情况作为额外参考；它不是个人安全预测。 | 研究功能 · 方法 3.0 |
| 方案 A 与方案 B 并排比较 | 任意选择两条页面中的路线，并排查看预计时间、实时依据、步行、换乘、换乘余量、稳定性、历史背景和道路背景。 | 不用反复切换卡片，就能比较“更快但换乘较紧”和“稍慢但直达”的区别。 | 已上线，测试版 |
| 目的地停车压力 | 把附近收费车位清单与近期付费活动合成相对参考；匹配到地图停车表的记录不足 70% 时，不输出“高/低”。它不等于占用率或空位数。 | 到 Mission 接人前查看付费活动；覆盖率下降时仍可看原始数量，但不会看到夸大的等级。 | 研究功能 |
...
| 安全优先 | 预计时间 + 当前稳定性 + 步行 + 换乘 + `max(百分位 − 50, 0) × 0.15` | 希望历史背景更明显地参与排序，并理解它不是安全预测 |

Safety 方法 3.0 在 v1 系列中固定不再漂移。百分位基准包含全部 Muni 站点，而不是只比较曾经出现事件的网格。直达路线使用“起点/上车 45%、沿途 40%、目的地 15%”；一次换乘使用“起点/上车 20%、沿途 35%、换乘 30%、目的地 15%”。缺少某部分时删除该部分并重新分配剩余权重；每部分最高按第 95 百分位计入，避免一个极端地点支配整趟路线。

v1 规划器有意限定为最多一次换乘。走廊级历史行程速度基线安排在 v1.1；当前的速度提示只把最新上报的移动速度与公开的车辆类型参考值比较，不会把它写成“历史平均速度”。
...
核心车辆位置与班次预测计划每 5 分钟更新；服务通知和道路背景每 15 分钟、停车每 30 分钟、安全与静态 GTFS 每天更新。核心计划预计每小时使用 32 次 511 请求，在默认每小时 60 次限制内保留 28 次余量。独立监控每 15 分钟同时检查 `generated_at` 和公交数据本身的 `observed_at`；任意一个超过 12 分钟就执行恢复刷新。超过 10 分钟的预测会自动降级为估算。
...
- 经常更新的公交核心文件约 0.5 MB；道路、停车、安全和静态路网分别保存，浏览器不必每次重新下载完整旧快照。
