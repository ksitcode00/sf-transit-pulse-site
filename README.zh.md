...
| 安全优先 | 比较候选行程附近过去 365 天的历史事件报告相对值；三种模式都会先查看完整可行候选，再缩短页面列表。 | 夜间出行时，把沿途历史情况作为额外参考；它不是个人安全预测。 | 研究功能 |
...
核心车辆位置与班次预测计划每 5 分钟更新；服务通知和道路背景每 15 分钟、停车每 30 分钟、安全与静态 GTFS 每天更新。核心计划预计每小时使用 32 次 511 请求，在默认每小时 60 次限制内保留 28 次余量。GitHub Actions 可能延迟或漏掉定时任务，因此超过 10 分钟的预测会自动降级为估算，页面也会显示真实数据年龄。
