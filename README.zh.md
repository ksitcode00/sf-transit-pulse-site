...
| 实时候选路线 | 比较直达和一次换乘；先保留所有空间上可行的换乘点，再用具体班次决定稍远站点是否反而更快。 | 不能因为一个换乘点不是最近的八个之一，就提前删掉真正最快的方案。 |
...
| 安全优先 | 比较去重后的 30、90、365 天历史报告，并使用透明的事件类别权重。起点、上车、沿途、换乘、目的地分别计权，同时限制单个极端地点的影响。 | 夜间出行时，把沿途历史情况作为额外参考；它不是个人安全预测。 | 研究功能 |
...
| 数据新鲜度与失败状态 | 每个数据源都有“可使用/已过期”状态；每条路线都写明“实时预测”“实时数据有限”或“估算时间”；停车会显示最后可用记录。 | 511 暂停更新时，不会静默地把估算包装成实时结果。 | 已上线 |
| 稳定的自动刷新 | 新快照到达后会重新计算当前行程，但保留用户正在看的路线；最佳方案变化时会明确提示。 | 五分钟更新一次数据，也不会突然把页面跳到另一条行程。 | 已上线 |
...
- 回归测试专门覆盖“第九个换乘点获胜”：即使它不是最近八个之一，只要能接上更早的具体班次，就不能被提前删掉。
